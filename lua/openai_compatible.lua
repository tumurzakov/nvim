...
local function get_models(self, opts)
  ...
  log:info("Fetching OpenAI compatible models from: " .. url)
...
local adapter = {
  ...
  handlers = {
    ...
    ---@return boolean
    setup = function(self)
      log:info("Setting up OpenAI Compatible adapter")
      if self.opts and self.opts.stream then
        self.parameters.stream = true
        self.parameters.stream_options = { include_usage = true }
      end
      print("--- diagnostic: REQUEST TARGET ---")
      print("URL/Endpoint: " .. (self.endpoint or self.url))
      print("Headers: " .. vim.inspect(self.headers))
      log:info("Adapter URL/Endpoint: " .. (self.endpoint or self.url))
      log:info("Adapter Headers: " .. vim.inspect(self.headers))
      return true
    end,
    ...
    form_parameters = function(self, params, messages)
      log:info("OpenAI Compatible Adapter: Forming parameters: " .. vim.inspect(params))
      print("--- diagnostic: REQUEST PARAMETERS ---")
      print(vim.inspect(params))
      return openai.handlers.form_parameters(self, params, messages)
    end,
    form_messages = function(self, messages)
      log:info("OpenAI Compatible Adapter: Forming messages: " .. vim.inspect(messages))
...
return {
  extend = function(_, overrides)
    return vim.tbl_deep_extend("force", adapter, overrides)
  end,
}
